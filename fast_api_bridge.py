...
async def lifespan(app: FastAPI):
    ...
    node = mqtt_data_uploader_t("147.83.52.40")
...
class robot_data_01_model(BaseModel):
    ...
    crop_type_data: Optional[List[str]] = None

    ambient_temperature_data: Optional[List[float]] = None
    relative_humidity_data: Optional[List[float]] = None
    absolute_humidity_data: Optional[List[float]] = None
    dew_point_data: Optional[List[float]] = None
    ...
    for table in query_result:
        for record in table.records:
            ...
            entry = {
                ...
                "ndvi_ir_data": row.get("ndvi_ir_data"),
                "ndvi_visible_data": row.get("ndvi_visible_data"),
                "area_segmentation_data": row.get("area_segmentation_data"),
                "location_data": row.get("location_data"),
                "biomass_data": row.get("biomass_data"),
                "crop_light_state_data": row.get("crop_light_state_data"),
                "crop_type_data": row.get("crop_type_data"),
                "ambient_temperature_data": row.get("ambient_temperature_data"),
                "relative_humidity_data": row.get("relative_humidity_data"),
                "absolute_humidity_data": row.get("absolute_humidity_data"),
                "dew_point_data": row.get("dew_point_data"),
